...
calculate_btn.onclick = () => {
        Fname = document.getElementById('boyfriend').value.toUpperCase();  
        FnameLength = Fname.length;  
         
        Sname = document.getElementById('girlfriend').value.toUpperCase();  
        ...
        for (var count = 0; count < SnameLength; count++){  
            ...
            let love_score = document.getElementById('love_score') 
            love_score.innerText = Math.floor(Total).toString();  
            // console.log("Toatal" + Math.floor(Total)); 
        }else{
